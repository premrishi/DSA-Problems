...
public class BottomViewoftree {
    ...
    public static void bottomViewOfBinaryTree(Node node) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (removedNode.left != null) {
                queue.add(new PairClassB<>(removedNode.left, removedNodeVerticalDistance - 1));
            }

            // Enqueue the right child with an increased vertical distance
            if (removedNode.right != null) {
                queue.add(new PairClassB<>(removedNode.right, removedNodeVerticalDistance + 1));
            }
        }
    ...
    public static class PairClassB<N, I extends Number> {
    	BottomViewoftree.Node node;
        int verticalDistance;
        public PairClassB(BottomViewoftree.Node node, int verticalDistance) {
            this.node = node;
